...
def getPatientId( studyPatientTissue ):
    ...
    patient_result = componentsCollection.find({'projectRun': {'$regex' : project_run_search}},{'kb.patients.studyPatientID':1})
    id = ""
...
def getCancerCensus():
    cancer_census_genes = geneCollection.find({'cancerCensus': {"$exists": "true"}},{"gene":1})
    cc_genes = []
...
def getVariants():
    ...
    studyResult = tumorCollection.find({"variants.study" : study, 'variants.filter' : 'PASS', "gene" : { "$in": ccGenes},
                                    ...
                                    "$or": [{'variants.filetype' : 'seuratPointMutationFile'},
                                                      {'variants.filetype' : 'freebayesFile'}],
                                    "$or": [
                                            {'aberration.aberration_type':'SNV'},
